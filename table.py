...
class Table:
    ...
    def handle_mouse_click(self, pos):
        ...
        if x <= WIDTH and y <= HEIGHT:
            ...
            elif clicked_cell.value != 0 and clicked_cell.value != \
                    self.answers[y][x]:
        ...
        # deleting numbers
        elif x <= (CELL_SIZE[0] * 3) and y >= (HEIGHT + CELL_SIZE[1]) and y <= (
                HEIGHT + CELL_SIZE[1] * 2):
            if self.cell_to_empty:
                self.cell_to_empty.value = 0
                self.cell_to_empty = None
        # selecting modes
        elif x >= (CELL_SIZE[0] * 6) and y >= (HEIGHT + CELL_SIZE[1]) and y <= (
                HEIGHT + CELL_SIZE[1] * 2):
        ...
        if self.clicked_num_below and self.clicked_cell != None and self.clicked_cell.value == 0:
            ...
            if self.guess_mode:
                ...
                if self._not_in_row(current_row,
                                    self.clicked_num_below) and self._not_in_col(
                    current_col, self.clicked_num_below):
                    if self._not_in_subgroup(rowstart, colstart,
                                             self.clicked_num_below):
                        if self.clicked_cell.guesses != None:
                            self.clicked_cell.guesses[
                                self.clicked_num_below - 1] = self.clicked_num_below
            else:
                self.clicked_cell.value = self.clicked_num_below
                # if the player guess correctly
                if self.clicked_num_below == \
                        self.answers[self.clicked_cell.col][
                            self.clicked_cell.row]:
    ...
    def update(self):
        [cell.update(self.screen, self.SRN) for cell in
         self.table_cells]
        ...
        self.screen.blit(self.clock.display_timer(),
                         (WIDTH // self.SRN, HEIGHT + CELL_SIZE[1]))
